...
const checks = [
  ...
  ["Paid beta path drafted", "Pricing and beta-interest capture are staged before payment checkout connects."]
];
...
const betaName = document.querySelector("#betaName");
const betaEmail = document.querySelector("#betaEmail");
const betaUseCase = document.querySelector("#betaUseCase");
    ...
    .map((project) => `
      <article class="project-card">
        <div>
          ...
          <p>${escapeHtml(project.type)} - ${escapeHtml(project.status)}</p>
        </div>
...
function applyBetaState() {
  ...
  betaEmail.value = state.betaEmail;
  betaUseCase.value = state.betaUseCase;
  selectedPlan.textContent = state.selectedPlan ? `${state.selectedPlan} selected` : "No plan selected";
  planChoiceButtons.forEach((button) => {
    button.classList.toggle("selected", button.dataset.planChoice === state.selectedPlan);
  });
}
...
planChoiceButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (!state.completeChecks.includes(6)) state.completeChecks.push(6);
    applyBetaState();
    renderChecks();
    updateReadiness();
    saveState();
    betaStatus.textContent = `${state.selectedPlan} interest saved locally. Connect Stripe checkout before charging customers.`;
  });
});

betaForm.addEventListener("submit", (event) => {
  event.preventDefault();
  state.betaName = betaName.value.trim();
  state.betaEmail = betaEmail.value.trim();
  state.betaUseCase = betaUseCase.value.trim();
  if (!state.completeChecks.includes(6)) state.completeChecks.push(6);
  applyBetaState();
  renderChecks();
  updateReadiness();
  saveState();
  betaStatus.textContent = "Beta interest saved locally. Backend storage and payment checkout are the next production step.";
});
